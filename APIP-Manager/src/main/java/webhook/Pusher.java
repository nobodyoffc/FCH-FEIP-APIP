...
public class Pusher implements Runnable{
    ...
    private Map<String,Map<String, WebhookUser>> methodFidEndpointInfoMapMap = new HashMap<>();
    ...
    public void run() {
        ...
        try(Jedis jedis = new Jedis()) {
            ...
            List<WebhookUser> webhookInfoList;
    ...
    private void readMethodFidWebhookInfoMapMapFromRedis(Jedis jedis) {
        ...
        Map<String, WebhookUser> newCashByFidsHookInfoMap = new HashMap<>();
        for(String owner: newCashByFidsHookInfoStrMap.keySet()){
            String webhookInfoStr = newCashByFidsHookInfoStrMap.get(owner);
            newCashByFidsHookInfoMap.put(owner,gson.fromJson(webhookInfoStr, WebhookUser.class));
        }
        methodFidEndpointInfoMapMap.put(ApiNames.NewCashByFidsAPI,newCashByFidsHookInfoMap);
        //More method:
    }

    private void setNewCashByFidsMapMapIntoRedis(Map<String, Map<String, WebhookUser>> methodFidEndpointInfoMapMap, Jedis jedis) {
        ...
        for(String method:methodFidEndpointInfoMapMap.keySet()){
            Map<String, WebhookUser> ownerWebhookInfoMap = methodFidEndpointInfoMapMap.get(method);
            for(String owner:ownerWebhookInfoMap.keySet()){
                WebhookUser webhookInfo = ownerWebhookInfoMap.get(owner);
                jedis.hset(method,owner,gson.toJson(webhookInfo));
            }
        }
    }

    private void makeMethodFidEndpointMapMap(List<WebhookUser> webhookInfoList) {
        if(webhookInfoList==null || webhookInfoList.size()==0)return;
        Map<String, WebhookUser> newCashByFidsWebhookInfoMap = new HashMap<>();
        for (WebhookUser webhookInfo : webhookInfoList) {
    ...
    private List<WebhookUser> getWebhookInfoListFromEs(ElasticsearchClient esClient) {
        try {
            return EsTools.getAllList(esClient,StartAPIP.serviceName.toLowerCase()+"_"+IndicesNames.WEBHOOK, Strings.HOOK_ID,SortOrder.Asc, WebhookUser.class);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void pushWebhooks(Map<String, Map<String, WebhookUser>> methodFidWatchedFidsMapMap, long sinceHeight) {
    ...
    private void putNewCashByFids(Map<String, WebhookUser> ownerWebhookInfoMap, long sinceHeight) {
        for(String owner:ownerWebhookInfoMap.keySet()){
            WebhookUser webhookInfo = ownerWebhookInfoMap.get(owner);
    ...
    private void pushNewCashList(WebhookUser webhookInfo, ArrayList<Cash> newCashList) {
        Gson gson = new Gson();
    ...
    private ArrayList<Cash> getNewCashList(WebhookUser webhookInfo, long sinceHeight) {
        Gson gson = new Gson();
