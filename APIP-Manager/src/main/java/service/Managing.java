...
public class Managing {
	...
	ApipService service = new ApipService();
	...
	private ApipService getService(Jedis jedis, ElasticsearchClient esClient, BufferedReader br) throws IOException {
		Gson gson = new Gson();

        System.out.println("Input the fch address of the owner:");
        String str = br.readLine();
        if(str.equals(""))return null;
        SearchResponse<ApipService> result = esClient.search(s -> s.index(ServiceIndex).query(q -> q.term(t -> t.field("owner").value(str))), ApipService.class);
...
//		}
			...
			GetResponse<ApipService> result = esClient.get(g->g.index(ServiceIndex).id(sid), ApipService.class);
