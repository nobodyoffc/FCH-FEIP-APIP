...
@WebServlet(ApiNames.APIP16V1Path + ApiNames.TokenHistoryAPI)
public class TokenHistory extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //Request
        String index = IndicesNames.TOKEN_HISTORY;

        DataRequestHandler esRequest = new DataRequestHandler(dataCheckResult.getAddr(),requestBody,response,replier);
        List<feipClass.TokenHistory> meetList;
        try {
            meetList = esRequest.doRequest(index,sort, feipClass.TokenHistory.class);
            if(meetList==null){
                return;
            }
        } catch (Exception e) {
