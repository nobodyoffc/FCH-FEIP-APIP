...
@WebServlet(ApiNames.APIP16V1Path + ApiNames.TokenHolderByIdsAPI)
public class TokenHolderByIds extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            meetList = esRequest.doRequest(index,null, TokenHolder.class);
            if(meetList==null){
                return;
            }
        } catch (Exception e) {
        ...
        Map<String,TokenHolder> meetMap = new HashMap<>();
        for(TokenHolder token :meetList){
