...
@WebServlet(ApiNames.APIP21V1Path + ApiNames.OffLineTxByCdAPI)
...
//            replier.setData("No cashes meet the request: "+cd+" cd and "+amount/ FchToSatoshi +" F.");
            writer.write(replier.reply1020OtherError(addr));
            return;
        }
        ...
        if(totalValue<amount+1000){
            response.setHeader(ReplyInfo.CodeInHeader, String.valueOf(ReplyInfo.Code1020OtherError));
            replier.setData("Cashes meeting this cd can not match the total amount of outputs.");
            writer.write(replier.reply1020OtherError(addr));
            return;
        }
