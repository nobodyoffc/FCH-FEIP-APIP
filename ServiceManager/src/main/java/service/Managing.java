...
public class Managing {
	...
	public void menu(ElasticsearchClient esClient, BufferedReader br, Jedis jedis) throws IOException {
		...
		while(true) {
			...
			switch (choice) {
				case 1:
					...
					break;
				case 2:
					getService(jedis, esClient, br);
					break;
				case 3:
					if (jedis.get(SERVICE_NAME + Strings.SERVICE_OBJECT) == null) {
						System.out.println("No service set yet.");
						br.readLine();
						break;
					}
					Gson gson = new Gson();
					Service service1 = gson.fromJson(jedis.get(SERVICE_NAME + Strings.SERVICE_OBJECT), Service.class);
	...
	private void reloadService(ElasticsearchClient esClient, Jedis jedis) throws IOException {
		...
		if(serviceStr==null){
			...
			service = new Gson().fromJson(serviceStr, Service.class);
			GetResponse<Service> r = esClient.get(g -> g.index("service").id(service.getSid()), Service.class);
			if(r.source()!=null) {
	...
	private Service getService(Jedis jedis, ElasticsearchClient esClient, BufferedReader br) throws IOException {
		Gson gson = new Gson();
		...
		private void publish(BufferedReader br, Jedis jedis) throws IOException {
			...
			ask = "Input the types of your service. Press enter to end :";
			String[] types = inputStringArray(br,ask,0);
			if(types.length!=0)data.setTypes(types);
		...
		private String[] inputStringArray(BufferedReader br, String ask, int requiredStringLength) throws IOException {
			...
			while(true) {
				...
				if(requiredStringLength>0) {
					if(item.length()!=requiredStringLength) {
		...
		private Service checkService() {
			String serviceStr;
			try {
				serviceStr = jedis.hget(IndicesNames.SERVICE, Strings.SID);
				service = new Gson().fromJson(serviceStr, Service.class);
			}catch (Exception e){
				System.out.println("Service isn't set yet.");
				return null;
