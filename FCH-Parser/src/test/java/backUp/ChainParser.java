...
public class ChainParser {
	...
	public int startParse(ElasticsearchClient esClient) throws Exception {
		...
		while(true) {
			...
			if(Preparer.orphanList.size()>10000)recoverTo(esClient,Preparer.BestHeight-1);
	...
	private void recoverTo(ElasticsearchClient esClient, long toHeight) throws Exception {
		SearchResponse<BlockMark> response = esClient.search(s->s.index(IndicesFCH.BlockMarkIndex)
						.query(q->q.term(t->t.field("height").value(toHeight)))
				, BlockMark.class);
		...
		if(backToBlockMark!=null) {
			...
			Preparer.mainList.removeIf(mainMark -> mainMark.getHeight() > toHeight);
			Preparer.forkList.removeIf(forkMark -> forkMark.getHeight() > toHeight);
			Preparer.orphanList.removeIf(orphanMark -> orphanMark.getOrphanHeight() > toHeight);
		}
	}
		...
		while(!found) {	
			...
			for(int i= 0;i<Preparer.orphanList.size();i++) {
				...
				if(blockMark.getPreBlockId().equals(Preparer.BestHash)) {
					...
					if(!bestBlockMark.getBlockId().equals(Preparer.BestHash)) {
