...
public class Aes256CbcP5 {

	    public static void main(String[] args) throws Exception {
	        // Read the encrypted data from a file
	        byte[] cipherBytes = Files.readAllBytes(Paths.get("encrypted.txt"));

	        // Read the key and IV from files
	        byte[] symKeyBytes0 = Files.readAllBytes(Paths.get("key.txt"));
	        byte[] ivBytes0 = Files.readAllBytes(Paths.get("iv.txt"));

	        // Convert key and IV hex strings to bytes
	        String keyHex = new String(symKeyBytes0, StandardCharsets.UTF_8);
	        String ivHex = new String(ivBytes0, StandardCharsets.UTF_8);
